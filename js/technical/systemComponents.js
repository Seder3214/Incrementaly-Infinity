var systemComponents = {
	...
	'overlay-head': {
		...
		<br>
		<span v-if="player.points.lt('1e1000') && options.ru == false "  class="overlayThing">You have </span>
		<span v-if="player.points.lt('1e1000') && options.ru == true "  class="overlayThing">У вас </span>
		<h2  class="overlayThing" id="points">{{format(player.points)}}</h2>
		<span v-if="player.points.lt('1e1e6') && options.ru == false "  class="overlayThing"> {{modInfo.pointsName}}</span>
		<span v-if="player.points.lt('1e1e6') && options.ru == true"  class="overlayThing"> очков</span>
		<br>
		<span v-if="canGenPoints()  && options.ru == false"  class="overlayThing">({{tmp.other.oompsMag != 0 ? format(tmp.other.oomps) + " OOM" + (tmp.other.oompsMag < 0 ? "^OOM" : tmp.other.oompsMag > 1 ? "^" + tmp.other.oompsMag : "") + "s" : formatSmall(getPointGen())}}/sec)</span>
<span v-if="canGenPoints()  && options.ru == true"  class="overlayThing">({{tmp.other.oompsMag != 0 ? format(tmp.other.oomps) + " OOM" + (tmp.other.oompsMag < 0 ? "^OOM" : tmp.other.oompsMag > 1 ? "^" + tmp.other.oompsMag : "") + "s" : formatSmall(getPointGen())}}/сек)</span>
        ...
        <table>
            ...
            <tr>
                ...
                <td><button class="opt" onclick="toggleOpt('forceOneTab'); needsCanvasUpdate = true">Single-Tab Mode: {{ options.forceOneTab?"ALWAYS":"AUTO" }}</button></td>
				 <td><button class="opt" onclick="toggleOpt('ru')">Translate to Russian: {{ options.ru?"ON":"OFF" }}</button></td>
			</tr> 
        </table>`
    },
